...
| **3. Run Maven Build** | Jenkins runs Maven to compile the code, build the microservice API JAR file, and install dependencies. | `mvn clean package` |
| **4. Run Unit Tests with JUnit** | Jenkins triggers **JUnit** to execute unit tests and ensure the correctness of the microservice code. | `mvn test` |
| **5. SonarQube Scan** | **SonarQube** is used to analyze the source code for quality, maintainability, and security vulnerabilities before further processing. | `sonar-scanner -Dsonar.projectKey=<project-key> -Dsonar.host.url=<sonar-url> -Dsonar.login=<token>` |
