...
| **Infrastructure as Code** | - All resources (EKS cluster, launch templates, node groups, add-ons) are defined using **Terraform**.<br>- **Terraform `null_resource`** with `local-exec` is used to run imperative tasks like draining nodes.<br>- **GitLab deployment pipelines** are used for deploying infrastructure updates. |
| **Pre-Upgrade Validation** | - Upgrade is first tested in a **staging environment**.<br>- Ensures compatibility with new Kubernetes version, add-on stability, and AMI behavior. |
| **Control Plane Upgrade**  | - Upgrade the control plane using `eksctl`. <br> `eksctl upgrade cluster --name gl-dev-cluster --region us-east-1 --kubernetes-version 1.30`<br>- Monitor until the cluster status becomes `ACTIVE` using `eksctl` or `awscli`. <br> `aws eks describe-cluster --name gl-dev-cluster --region us-east-1 --query 'cluster.status'` |
| **Blue-Green Node Upgrade** | - Create a **new Launch Template** with the updated AMI version.<br>- Launch a new **Auto Scaling Group (ASG)** using **Terraform** and **`eksctl`**.<br> `eksctl create nodegroup --cluster gl-dev-cluster --name new-nodegroup --node-type t3.medium --nodes 2 --kubernetes-version 1.30 --region us-east-1`<br>- **Update ASG** via Terraform: <br> `aws autoscaling update-auto-scaling-group --auto-scaling-group-name <asg-name> --launch-template LaunchTemplateId=<template-id>,Version=<new-version>`<br>- Add new nodes gradually while old nodes are **cordoned and drained** using `kubectl` and **Terraform `null_resource`**.<br> `kubectl cordon <node-name>` and `kubectl drain <node-name> --ignore-daemonsets --delete-emptydir-data`<br>- **Terminate old nodes** only after confirming workload health with `awscli`.<br> `aws autoscaling terminate-instance-in-auto-scaling-group --instance-id <old-instance-id> --no-should-decrement-desired-capacity` |
| **Add-on Management**      | - Upgrade add-ons like `vpc-cni`, `kube-proxy`, and `coredns` using `awscli`.<br> `aws eks update-addon --cluster-name gl-dev-cluster --region us-east-1 --addon-name vpc-cni --resolve-conflicts OVERWRITE`<br>- Verify with `awscli` or `eksctl`.<br> `aws eks describe-addon --cluster-name gl-dev-cluster --region us-east-1 --addon-name <addon-name>` |
| **Monitoring & Observability** | - Use **Prometheus**, **Grafana**, and **CloudWatch** to monitor node/pod health and resource metrics.<br>- Enable alerts for node readiness, pod failures, and high latencies. |
| **Pod Health Validation**  | - All pods are configured with **readinessProbes** and **livenessProbes**.<br>- Ensures Kubernetes only sends traffic to healthy containers. |
| **Post-Deployment Health Check** | - Use **ALB Ingress Controller** integrated with Kubernetes to expose services externally.<br>- Verify **AWS ALB target group health checks** to confirm traffic routing to only healthy pods.<br> `aws elbv2 describe-target-health --target-group-arn <target-group-arn>`<br>- Perform smoke tests and endpoint checks after node rollout. |
| **Rollback Strategy**      | - Keep older AMIs and Launch Template versions intact.<br>- Rollback is as simple as reverting GitLab changes and reapplying Terraform. |
| **Post-Upgrade Validation** | - Confirm node status with `kubectl get nodes`.<br>- Validate services using readiness and liveness probes, ALB health, and manual or automated application checks. |
